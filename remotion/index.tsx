...
import { AbsoluteFill, Composition, Html5Audio, OffthreadVideo, Sequence, registerRoot, staticFile, useVideoConfig } from 'remotion';
...
const Scene: React.FC<{ clip: DemoVideoProps['clips'][number] }> = ({ clip }) => {
  ...
  const target = clip.presentation.camera.to;
  const cameraScale = clip.presentation.camera.type === 'zoom'
    ? clip.presentation.camera.scale ?? 1.15
    : clip.presentation.camera.type === 'crop' && target
      ? Math.min(3, Math.max(1 / target.width, 1 / target.height))
      : clip.presentation.camera.type === 'pan' ? 1.04 : 1;
  const transformOrigin = target ? `${(target.x + target.width / 2) * 100}% ${(target.y + target.height / 2) * 100}%` : 'center';
  ...
  return <AbsoluteFill style={{ background: clip.presentation.caption.mode === 'none' ? '#000' : '#07111f', overflow: 'hidden' }}>
    ...
    <OffthreadVideo muted src={staticFile(clip.src)} style={{ width: '100%', height: '100%', objectFit: 'cover', scale: cameraScale, transformOrigin, background: '#07111f' }} />
    {clip.presentation.caption.mode === 'lower-third' ? <AbsoluteFill style={{ zIndex: 1, pointerEvents: 'none' }}>
      <div style={{ position: 'absolute', left: width * 0.04, right: width * 0.2, top: captionTop ? height * 0.035 : undefined, bottom: captionTop ? undefined : height * 0.035, padding: `${Math.max(10, height * 0.014)}px ${Math.max(14, width * 0.014)}px`, borderRadius: 14, color: 'white', background: '#06101dda', borderLeft: `6px solid ${clip.purpose === 'proof' || clip.purpose === 'result' ? '#34d399' : clip.presentation.transitionWeight === 'major' ? '#f59e0b' : '#60a5fa'}`, fontFamily: 'Inter, system-ui, sans-serif', boxShadow: '0 12px 40px #0007' }}>
